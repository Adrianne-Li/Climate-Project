...
def main():
    ...
    parser.add_argument(
        "--osf-project",
        default=None,
        help="Public OSF project GUID/URL to use as primary data source",
    )
    parser.add_argument("--osf-config", default=None)
    parser.add_argument(
        "--osf-token",
        default=None,
    ...
    if not args.skip_matcher:
        ...
        if args.osf_project:
            matcher_argv += ["--osf-project", args.osf_project]
        if args.osf_config:
            matcher_argv += ["--osf-config", args.osf_config]
        if args.osf_token:
            matcher_argv += ["--osf-token", args.osf_token]
