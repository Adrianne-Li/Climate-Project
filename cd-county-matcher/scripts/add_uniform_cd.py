#!/usr/bin/env python3
"""
Pipeline step 2 of 3: add a uniform CD number + apply the one-year backward
year shift.
...
2. Shifts every row's `year` backward by 1 to correct the data-storage glitch:
   the CD geometry stored under year Y actually reflects the district
   boundaries in force during session Y-1. After shifting, 2012 -> 2011, etc.

   IMPORTANT: the uniform CD extraction runs on the ORIGINAL year label,
   because the `year <= 2012` branch is tied to the original conventions.
   Shifting happens after.

    python scripts/add_uniform_cd.py
    python scripts/add_uniform_cd.py --input data/results/matches_state_filled.csv \
        --output data/results/matches_with_uniform_cd_shifted.csv --year-shift -1
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="Integer year shift applied as a glitch correction (default: -1)",
    )
