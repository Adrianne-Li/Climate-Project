...
def discover_osf_files(
    ...
    """List a project's OSF Storage and return ``{filename: download_url}``.

    Uses the public OSF API v2:
        GET https://api.osf.io/v2/nodes/<project_guid>/files/osfstorage/

    Each returned file carries ``attributes.name`` and a ready-to-use
    ``links.download`` URL. No authentication is needed for public projects;
    pass ``token`` (an OSF Personal Access Token) for private ones.

    Returns an empty dict on any failure (network, 404, private without token)
    ...
    # WaterButler base for direct downloads — using this instead of
    # links.download avoids a osf.io -> files.osf.io redirect that causes
    # requests to strip the Authorization header on the hostname change.
    wb_base = f"https://files.osf.io/v1/resources/{project_guid}/providers/osfstorage"

    file_map: Dict[str, str] = {}
    url = f"{OSF_API_BASE}/nodes/{project_guid}/files/osfstorage/"
    pages = 0
    try:
        while url and pages < 25:
            ...
            for item in payload.get("data", []):
                ...
                if kind == "file" and name:
                    # Prefer the direct WaterButler URL (no redirect, auth works).
                    # Fall back to links.download if path is unavailable.
                    if path:
                    ...
                    # Recurse one level into subfolders, if any.
                    sub = (
                    ...
                    if sub:
                        try:
                            r2 = sess.get(sub, headers=headers, timeout=30)
                            if r2.status_code == 200:
                                for c in r2.json().get("data", []):
                                    ca = c.get("attributes", {})
                                    cpath = ca.get("path", "")
                                    clinks = c.get("links", {})
                                    if ca.get("kind") == "file" and ca.get("name"):
                                        if cpath:
                                            file_map[ca["name"]] = f"{wb_base}{cpath}"
                                        elif clinks.get("download"):
                                            file_map[ca["name"]] = clinks["download"]
                        except Exception:
                            pass
...
def load_osf_project_guid(config_path: Optional[str] = None) -> Optional[str]:
    ...
    candidate_paths = [Path(__file__).resolve().parent.parent / "osf_sources.json"]
    if config_path:
...
class CDCountyMatcher:
        ...
        for attempt in range(max_retries):
            try:
                response = self.session.get(
                    url, timeout=600, verify=False, stream=True, headers=headers
                )
    ...
    def _load_newberry_full(self) -> Optional[gpd.GeoDataFrame]:
        ...
        # Also try auto-discovery from the configured OSF project.
        project_guid = load_osf_project_guid()
        if project_guid:
            osf_files = discover_osf_files(
                project_guid,
            ...
            if wanted and wanted in osf_files:
                urls.append((wanted, osf_files[wanted]))
        for pattern in [
            "US_HistCounties.zip",
