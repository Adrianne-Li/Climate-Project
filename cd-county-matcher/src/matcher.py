...
# ---------------------------------------------------------------------------
# Sources for the "manual" shapefiles that are too large/awkward to ship in
# the git repo. These used to be hardcoded local paths on the original
# author's machine.
#
# As of the OSF migration, the *primary* source for each of these is a copy
# stored in our project's OSF Storage, so we no longer hammer (and wait on)
# the original public servers — which were slow and whose URLs move around.
# `scripts/setup_data.py` fetches them into DATA_DIR/manual_sources/ on first
# run.
#
# How the source is chosen, per file:
#   1. If an OSF GUID is configured for the key (see OSF setup below), the
#      file is pulled from https://osf.io/<guid>/download.
#   2. Otherwise (or if the OSF fetch fails) we fall back to the original
#      canonical public URL recorded in "url".
#
# Configuring the OSF GUIDs (one-time, after uploading the zips to OSF):
#   - Easiest: edit osf_sources.json at the repo root and paste in the
#     5-character GUID for each file (osf.io/XXXXX/ -> "XXXXX").
#   - Or set the OSF_SOURCES_JSON env var to point at a JSON file elsewhere.
#   - Or hardcode a value in the "osf_guid" field below.
# See README.md ("Hosting the large files on OSF") for the upload walkthrough.
# ---------------------------------------------------------------------------
...
def osf_download_url(guid: str) -> str:
    """Build a direct-download URL for a single file given its OSF GUID.

    OSF assigns every file a stable 5-character GUID; appending ``/download``
    to ``https://osf.io/<guid>/`` returns the raw file regardless of where it
    sits in the project's folder tree or how many versions it has.
    """
    guid = str(guid).strip().strip("/")
    # Tolerate someone pasting a whole URL like "https://osf.io/abc12/".
    if "osf.io/" in guid:
        guid = guid.split("osf.io/")[-1].strip("/").split("/")[0]
    return f"{OSF_DOWNLOAD_BASE}/{guid}/download"


def _extract_guid(value: str) -> str:
    """Pull a bare GUID out of a value that may be a GUID or a full OSF URL."""
...
def discover_osf_files(
    ...
    """List OSF Storage and return ``{filename_or_path: download_url}``.

    Uses the public OSF API v2 and supports nested OSF folders. No
    authentication is needed for public projects; pass ``token`` only for
    private mirrors. Both bare filenames and OSF relative paths are inserted
    into the returned map, but the pipeline generally matches by bare filename
    such as ``districts113.zip`` or ``tl_2015_us_county.shp``.

    Returns an empty dict on any failure so callers can fall back to public
    online sources.
    """
    ...
    wb_base = f"https://files.osf.io/v1/resources/{project_guid}/providers/osfstorage"
    ...
    def walk(listing_url: str, depth: int = 0) -> None:
        ...
        while url and pages_seen < max_pages:
            ...
            try:
                ...
                print(f"  [osf] discovery error for project {project_guid}: {e}")
                return

            if resp.status_code != 200:
                if depth == 0:
                    print(
                        f"  [osf] file listing for project {project_guid} "
                        f"returned HTTP {resp.status_code} "
                        f"({'private? set OSF_TOKEN' if resp.status_code in (401, 403) else 'check the project GUID'})."
                    )
                return
            ...
            for item in payload.get("data", []):
                ...
                if kind == "file" and name:
                    if path:
                        ...
                        dl_url = links["download"]
                    else:
                        continue
                    # Match by bare filename, and also keep the OSF relative
                    # path for debugging / possible disambiguation.
...
def load_osf_project_guid(config_path: Optional[str] = None) -> Optional[str]:
    """Resolve the OSF *project* GUID for auto-discovery.

    Order: ``OSF_PROJECT_GUID`` env var, then ``_osf_project`` /
    ``_osf_project_guid`` in osf_sources.json (or the file at config_path /
    OSF_SOURCES_JSON). Accepts a bare GUID or a full osf.io URL.
    """
    import json
    ...
    env_path = os.environ.get("OSF_SOURCES_JSON")
    if env_path:
        candidate_paths.append(Path(env_path))
    candidate_paths.append(Path(__file__).resolve().parent.parent / "osf_sources.json")
        ...
        try:
            if path and path.exists():
                ...
                for key in ("_osf_project_guid", "_osf_project"):
                    val = data.get(key)
                    if val and "your-project" not in str(val):
                        guid = _extract_guid(val)
                        if guid:
                            return guid
        except Exception:
            pass
...
MANUAL_SOURCE_URLS = {
    ...
    "county_2010": {
        "osf_guid": "",  # <- paste OSF GUID here (or via osf_sources.json)
        "osf_filename": "tl_2010_us_county10.zip", # filename as uploaded to OSF
        ...
        "fallback_note": (
            "Primary source is OSF. If no OSF GUID is configured (and the "
            "Census fallback URL fails too), download "
            "tl_2010_us_county10.zip from www2.census.gov and unzip it to "
            "DATA_DIR/manual_sources/county_2010/"
        ),
    },
    # 2023 CD cartographic boundary (118th Congress) - reused for 2024.
    "cd_118th_2023": {
        "osf_guid": "",
        "osf_filename": "cb_2023_us_cd118_5m.zip", # filename as uploaded to OSF
        ...
        "fallback_note": (
            "Primary source is OSF. Fallback is the Census GENZ2023 CB file."
        ),
    },
    # 2025 / 119th Congress districts.
    "cd_119th_2025": {
        "osf_guid": "",
        "osf_filename": "cb_2024_us_cd119_5m.zip", # filename as uploaded to OSF
        ...
        "fallback_note": (
            "Primary source is OSF. If the OSF fetch is unconfigured and the "
            "Census fallback URL 404s, download the NTAD Congressional "
            "Districts shapefile from https://data.transportation.gov/ and "
            "unzip it to DATA_DIR/manual_sources/cd_119th_2025/"
        ),
    },
    # Newberry Atlas of Historical County Boundaries - full US file, ~200MB.
    # This is the big one whose original host (publications.newberry.org) was
    # the main reason setup_data.py used to crawl: hosting it on OSF is the
    # single biggest speedup.
    "newberry_historical": {
        "osf_guid": "",
        "osf_filename": "US_AtlasHCB_Counties.zip", # filename as uploaded to OSF
        ...
        "fallback_note": (
            "Primary source is OSF. If unconfigured and the download fails, "
            "visit https://publications.newberry.org/ahcb/pages/United_States.html "
...
def load_osf_guids(config_path: Optional[str] = None) -> Dict[str, str]:
    """Load per-source OSF GUIDs.

    Resolution order (later overrides earlier):
      1. ``osf_guid`` values hardcoded in MANUAL_SOURCE_URLS above.
      2. ``osf_sources.json`` at the repo root (if present).
      3. The JSON file pointed to by the ``OSF_SOURCES_JSON`` env var.

    Returns a ``{source_key: guid}`` dict containing only non-empty GUIDs.
    Keys starting with ``_`` in the JSON (comments) are ignored. Never raises
    on a missing/malformed file — it just falls back to what it has.
    """
    import json
    ...
    candidate_paths.append(repo_root / "osf_sources.json")
    if config_path:
        candidate_paths.append(Path(config_path))
    env_path = os.environ.get("OSF_SOURCES_JSON")
    if env_path:
        ...
        try:
            if path and path.exists():
                ...
                for key, value in data.items():
                    if key.startswith("_"):
                        continue  # comment / metadata field
                    if key in MANUAL_SOURCE_URLS and str(value).strip():
                        guids[key] = str(value).strip()
        except Exception as e:  # never let a bad config file break setup
            print(f"  [warn] could not read OSF config {path}: {e}")
...
class CDCountyMatcher:
    ...
    def __init__(
        ...
        # OSF is now the primary source. For a public OSF project, no token is
        # required. The token hook is kept only for backward compatibility with
        # private mirrors.
        ...
        if self.osf_project_guid:
            ...
            print("  [info] No OSF project configured; online public fallbacks will be used.")
    ...
    def _setup_congress_mappings(self) -> Dict[int, str]:
        """Map calendar years to UCLA congressional-cycle shapefiles.

        UCLA names files by Congress number, e.g. ``districts113.zip`` for the
        113th Congress. The service-date mapping is:
            1984 -> 098, 1985-1986 -> 099, ...,
            2013-2014 -> 113, 2015-2016 -> 114,
            2023-2024 -> 118, 2025 -> 119.
        This is equivalent to floor((year - 1789) / 2) + 1 for 1984-2025.
        """
        mappings = {}
    ...
    def _get_strategy(self, year: int) -> Dict[str, List[str]]:
        """OSF-first source order.

        Congressional districts now come from UCLA for every year/cycle in
        1984-2025. We first try the OSF mirror of the UCLA file
        (``districts098.zip`` ... ``districts119.zip``); only if OSF cannot be
        reached or the file is missing do we try UCLA's public download URL.
        """
    ...
    def _find_osf_file(self, filename: str) -> Optional[str]:
        """Return the OSF download URL for filename, case-insensitive."""
    ...
    def _download_file_with_retry(
        ...
        for attempt in range(max_retries):
            try:
                ...
                print(f"Download failed for {url}: {str(e)[:120]}")
                if filepath.exists():
                    filepath.unlink()
        return False

    def _download_osf_raw_components(self, stem: str, target_dir: Path) -> bool:
        """Download raw shapefile sidecars from OSF by basename/stem.

        XML metadata is intentionally ignored. A readable shapefile needs at
        least .shp + .shx + .dbf; .prj and .cpg are copied when present.
        """
        if not self.osf_files:
            return False
        ...
        for fname, url in sorted(found.items()):
            ok = self._download_file_with_retry(
                url, target_dir / fname, headers=self._osf_auth_headers(url)
            ) and ok
    ...
    def _download_osf_shapefile(
        ...
        """Try OSF zip(s), then raw shapefile components."""
        if not self.osf_files:
        ...
        for filename in zip_candidates:
            ...
            if self._download_and_extract_with_retry(
                url, filepath, headers=self._osf_auth_headers(url)
            ):
    ...
    def _download_cd(self, year: int, source: str) -> Tuple[bool, str]:
        ...
        if source == "osf_storage":
            ...
            pattern = f"districts{int(congress_num):03d}.zip"
            return self._download_osf_shapefile(
                [pattern], target_dir, [Path(pattern).stem]
            )
        if source == "manual_cd":
    ...
    def _download_county(self, year: int, source: str) -> Tuple[bool, str]:
        ...
        if source == "osf_storage":
            if year < 2000:
                # Pre-2000 counties are generated from the OSF-hosted Newberry
                # historical county shapefile.
                if self.newberry_loaded_gdf is None:
                    self.newberry_loaded_gdf = self._load_newberry_full()
                if self.newberry_loaded_gdf is not None:
            ...
            return self._download_osf_shapefile(
                zip_candidates, target_dir, raw_stems
            )
    ...
    def _load_newberry_full(self) -> Optional[gpd.GeoDataFrame]:
        ...
        # Fall back to a network download. Prefer our OSF-hosted copy (fast,
        # stable URL); only then try the original Newberry mirror, whose URLs
        # change periodically and are slow.
        target_dir = self.base_data_dir / "newberry_historical" / "full"
        target_dir.mkdir(exist_ok=True, parents=True)

        wanted = MANUAL_SOURCE_URLS["newberry_historical"].get("osf_filename")
        if wanted:
            ok, info = self._download_osf_shapefile(
                [wanted], target_dir, ["US_HistCounties"]
            )
            if ok:
                ...
                if shp_files:
                    ...
                    gdf["END_DATE"] = gdf["END_DATE"].fillna(datetime(2010, 12, 31))
                    print(f"Loaded Newberry data from OSF ({info})")
                    return gdf

        osf_guids = load_osf_guids()
        newberry_guid = osf_guids.get("newberry_historical")
        urls = []
        if newberry_guid:
            urls.append(("US_HistCounties_osf.zip", osf_download_url(newberry_guid)))
        for pattern in [
            "US_HistCounties.zip",
            ...
            urls.append((pattern, f"{self.newberry_base}/{pattern}"))

        osf_token = self.osf_token
        for pattern, url in urls:
            filepath = target_dir / pattern
            print(f"Trying Newberry URL: {url}")
            # Only attach the OSF token to OSF hosts (never to census/newberry).
            req_headers = (
                {"Authorization": f"Bearer {osf_token}"}
                if osf_token and "osf.io" in url
                else None
            )
            if self._download_and_extract_with_retry(
                url, filepath, headers=req_headers
            ):
                shp_files = list(target_dir.rglob("*.shp"))
                if shp_files:
                    gdf = gpd.read_file(shp_files[0])
